...
async function getRating(contentId: string) {
  ...
  let ratingResults;
  try {
    ratingResults = await fetchy(`/api/qualityRatings/${props.parent.id}`, "GET", { query });
  } catch (_) {
    return;
  }
...
const rate = async (contentId: string, rating: number) => {
  try {
    await fetchy(`/api/qualityRatings/${props.parent._id}`, "PUT", { body: { rating: newRating.value, contentId: contentId } });
  } catch (_) {
    return;
  }
};
